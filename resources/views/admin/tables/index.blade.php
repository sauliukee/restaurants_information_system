        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500">
            ...
            @foreach($tables as $table)
                <tr class="odd:bg-white even:bg-gray-50 border-b">
                    <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">{{ $table->name }}</td>
                    <td class="px-6 py-4">{{ $table->guest_number }}</td>
                    <td class="px-6 py-4">{{ $table->status->name }}</td>
                    <td class="px-6 py-4">{{ $table->location->name }}</td>
                    <td class="px-6 py-4">
                        {{ $table->restaurant ? $table->restaurant->name : 'N/A' }}
                    </td>
                    <td class="px-6 py-4">
